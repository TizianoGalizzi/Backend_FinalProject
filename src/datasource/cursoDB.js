...
cursosDB.inscAlumn = function(id,lista,funCallback){
    for(let i = 0; i<lista.alumnosID.length; i++){
        sqlConnection.query(`INSERT INTO alumno_curso (id_alumno,id_curso)VALUES
        (${lista.alumnosID[i]},${id});`, (err,result)=>{
            if(err){
                funCallback({
                    message: "Surgio un problema, contactese con un administrador. Gracias",
                    detail: console.log(err)
...
cursosDB.listID = function(id,funCallback){
    sqlConnection.query(`SELECT a.nombre, a.apellido, a.dni,a.id_usuario, c.nombre as curso FROM alumno_curso as a_c 
    INNER JOIN alumnos as a ON a_c.id_alumno = a.id
    INNER JOIN cursos as c ON a_c.id_curso = c.id  
    WHERE c.id = ${id} ORDER BY a.nombre ASC`,(err,result)=>{
        if(err){
            funCallback({
                message: "Surgio un problema, contactese con un administrador. Gracias",
                detail: console.log(err)
